    ...
    <header class="header">
      <div class="container header__container">
        ...
        <div class="header__option">
          <AddColumnButton @columnSubmitted="addNewColumn" />
...
import ModalWindowColumn from "@/components/ModalWindowColumn.vue";
...
export default defineComponent({
  ...
  computed: {
    ...mapState({
      tasks: (state) => state.boards.tasks,
      columns: (state) => state.boards.columns, 
    }),
  },
  methods: {
    ...mapActions(['loadTasks', 'addNewColumn']),
    addNewTask1(taskData) {
    },
    addNewColumn(columnData) {
      const boardId = this.$store.state.boards.currentBoardId;
      this.$store.dispatch('addNewColumn', { boardId, columnData });
    },
  },
  mounted() {
    this.loadTasks();
  ...
  created() {
    this.loadTasks(this.$store, this.$route.params.board_id); 
  },
});
</script>
